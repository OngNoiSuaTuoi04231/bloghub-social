  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        />
        {selected}
        <MI name={open ? "expand_less" : "expand_more"} className="text-[15px]" />
      </button>
...
function Waveform({ active, dark }) {
  const bars = [3, 6, 10, 14, 10, 7, 12, 16, 11, 8, 13, 9, 15, 10, 6, 11, 8, 14, 7, 10];
      ...
      <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6">
        ...
        >
          <div className="w-full">
            <h2 className={`text-2xl font-bold mb-2 ${dark ? "text-violet-200" : "text-slate-800"}`}>
              Create your moment
            </h2>

            <p className={`text-sm mb-8 ${dark ? "text-violet-500" : "text-slate-400"}`}>
            ...
            >
              {preview && mediaType.includes("image") ? (
                <img src={preview} alt="preview" className="w-full h-[520px] object-cover rounded-3xl" />
                ...
                <div className="text-center">
                  ...
                  <p className={`mt-4 font-semibold ${dark ? "text-violet-300" : "text-indigo-500"}`}>
        ...
        >
          ...
          <div className="flex-1 flex flex-col overflow-y-auto">
            <div className="flex items-center justify-between px-4 pt-4 pb-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-[11.5px] ${dark ? "text-violet-500" : "text-gray-400"}`}>
            ...
            <div className={`flex mx-4 mb-3 rounded-2xl p-1 gap-1 ${dark ? "bg-[#1e1535]" : "bg-gray-100"}`}>
              {[
            ...
            {activeTab === "photo" && (
              <div className="px-4 pb-3">
                  ...
                  {preview && mediaType.includes("image") ? (
                    <>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 px-4 py-3 bg-gradient-to-t from-black/60 to-transparent">
                        <p className="text-white font-bold text-sm text-center tracking-wide" style={{ fontFamily: "cursive" }}>
                          Safe chunder
                        </p>
                      </div>
                    </>
                    ...
                    <>
                      <div className={`absolute inset-8 rounded-full opacity-30 ${dark ? "bg-white/10" : "bg-white/60"}`} />
                      <div className={`absolute inset-16 rounded-full opacity-20 ${dark ? "bg-white/15" : "bg-white/80"}`} />
                      ...
                      <div className="relative z-10 flex flex-col items-center gap-2">
                        ...
                        >
                          <MI name="add_a_photo" className={`text-[28px] ${dark ? "text-violet-400" : "text-indigo-400"}`} />
                        </div>
                        <p className={`font-bold text-[14px] ${dark ? "text-violet-300" : "text-indigo-500"}`}>
                          Add Photo
                        </p>
                        <p className={`text-[10px] font-black tracking-[0.2em] uppercase ${dark ? "text-violet-600" : "text-indigo-300"}`}>
                          Locket Style
                        </p>
                      </div>
                ...
                >
                  <div className={`px-4 py-2 flex items-center gap-2 border-b ${dark ? "border-violet-800" : "border-indigo-100"}`}>
                    <MI name="photo_camera" className={`text-[17px] ${dark ? "text-violet-400" : "text-indigo-400"}`} />
                    <span className={`text-[12px] font-bold uppercase tracking-widest ${dark ? "text-violet-400" : "text-indigo-400"}`}>
                      Camera / Upload
                    </span>
                  </div>
                  <div className="p-3">
                    <CameraCapture onCapture={handleCapture} onCancel={() => {}} />
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === "voice" && (
              <div className="px-4 pb-3">
                ...
                >
                  {preview && mediaType === "voice_note" && (
                    <div className="px-4 pt-4 pb-2">
                      <audio src={preview} controls className="w-full h-10 rounded-xl" />
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3 px-4 py-4">
                    ...
                    <span className={`text-[12px] font-mono font-bold flex-shrink-0 ${dark ? "text-violet-400" : "text-indigo-400"}`}>
                      {fmtTime(recordTime)}
                    </span>
                  </div>

                  <div className="text-center pb-4">
                    <p className={`font-bold text-[13px] ${dark ? "text-violet-300" : "text-gray-700"}`}>
                      {isRecording ? "Đang ghi âm..." : "Record Voice"}
                    </p>
                    <p className={`text-[10px] font-black tracking-[0.2em] uppercase mt-0.5 ${dark ? "text-violet-600" : "text-indigo-300"}`}>
                      Audio Note
                    </p>
                  </div>
                </div>
                ...
                >
                  <div className={`px-4 py-2 flex items-center gap-2 border-b ${dark ? "border-violet-800" : "border-indigo-100"}`}>
                    <MI name="graphic_eq" className={`text-[17px] ${dark ? "text-violet-400" : "text-indigo-400"}`} />
                    <span className={`text-[12px] font-bold uppercase tracking-widest ${dark ? "text-violet-400" : "text-indigo-400"}`}>
                      Voice Recorder
                    </span>
                  </div>
