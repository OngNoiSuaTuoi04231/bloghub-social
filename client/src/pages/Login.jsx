...
function Login() {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 md:p-8 ${dark ? "bg-[#0d0820]" : "bg-gray-50"}`}>
      <DarkToggle dark={dark} onToggle={() => setDark((p) => !p)} />

      <div className={`flex w-full max-w-[920px] rounded-[32px] overflow-hidden ${dark ? "shadow-2xl shadow-violet-950/80" : "shadow-2xl shadow-indigo-200/60"}`}>
        
        {/* 1. PHẦN FORM ĐƯA LÊN TRƯỚC (SẼ Ở BÊN TRÁI TRÊN DESKTOP) */}
        <div className={`flex-1 flex flex-col justify-center px-7 py-9 md:px-12 ${dark ? "bg-[#130d28]" : "bg-white"}`}>
          <p className={`md:hidden text-center font-extrabold text-[27px] mb-5 ${dark ? "text-violet-400" : "text-indigo-500"}`}>
            VibeNest
          </p>

          <h1 className={`text-[30px] font-extrabold text-center ${dark ? "text-white" : "text-gray-900"}`}>
            Welcome Back
          </h1>

          <p className={`text-[13px] text-center mt-1 mb-7 ${dark ? "text-violet-400" : "text-gray-400"}`}>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-[14px]">
            ...
            <InputField
              ...
              dark={dark}
              rightEl={
                <button type="button" onClick={() => setShowPassword((p) => !p)} className={dark ? "text-violet-500" : "text-gray-400"}>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`mt-2 w-full py-[15px] rounded-2xl font-bold text-white transition-all duration-300 ${dark ? "bg-gradient-to-r from-violet-600 to-purple-600" : "bg-indigo-500 hover:bg-indigo-600"}`}
            >
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            <span className={`text-[13px] ${dark ? "text-violet-500" : "text-gray-500"}`}>
              You don’t have an account yet?
            </span>
            <Link to="/register" className={`text-[12.5px] font-semibold px-4 py-1.5 rounded-full border transition-all duration-300 ${dark ? "text-violet-300 border-violet-700 hover:bg-violet-900/50" : "text-indigo-500 border-indigo-200 hover:bg-indigo-50"}`}>
              Sign Up
            </Link>
          </div>
        </div>
